...
class Contact extends DatabaseObject {
  ...
  static protected $table_name = "contact_details";
  static protected $db_columns = ['id','image','mobile','whatsapp','facebook','twitter','linkedin','instagram','youtube','email','fax','landline','address'];
  public $id ;
  public $mobile;
  public $whatsapp;
  public $facebook;
...

  {
      ...
      $width = $fileinfo[0];
      $height = $fileinfo[1];
      $allowed_image_extension = array('png', 'jpg','svg');
      ...
      elseif ($file['error'] != 0) {
          ...
          $rand = rand(1111, 9999);
          $this->contact_image = date("YmdHis") . "-" . $rand . "." . $ext;
...

  {
      ...
      if ($this->id) {
          ...
          if (file_exists($target_path)) {
              $this->errors[] = "The file {$this->contact_image} already exists";
              return false;
          }
  ...
  static public function find_by_single_contact() {
    $sql = "SELECT * FROM " . static::$table_name . " ";
     $obj_array = static::find_by_sql($sql);
    if(!empty($obj_array)) {
      return array_shift($obj_array);
