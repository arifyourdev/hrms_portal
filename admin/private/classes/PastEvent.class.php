...
class PastEvent extends DatabaseObject {
static protected $table_name = "past_events";
static protected $db_columns = ['id','image','title','event_type','details','status','event_date','time'];
...
public $event_date;
public $tmp_path;
 public $upload_directory = "images/past_event";
  ...
  static public function find_by_all()
  {
   ...
   static public function find_by_web_events()
   {
      $sql = "SELECT * FROM " . self::$table_name . " ";
      $sql .= "WHERE event_type='WEBINARS'";
  ...
  public static function find_by_month($month,$event_type)
    {
        $sql = "SELECT * FROM " . self::$table_name . " ";
        $sql .= "WHERE MONTH(event_date)='" . self::$database->escape_string($month) . "' and event_type='" . self::$database->escape_string($event_type) . "'";
    ...
    public static function find_by_pastevent_month($month,$event_type)
    {
        $sql = "SELECT * FROM " . self::$table_name . " ";
        $sql .= "WHERE MONTH(event_date)='" . self::$database->escape_string($month) . "' and event_type='" . self::$database->escape_string($event_type) . "'";
        $sql .= "ORDER BY event_date desc";
   ...
   static public function find_by_work_events()
  {
      $sql = "SELECT * FROM " . self::$table_name . " ";
      $sql .= "WHERE event_type='WORKSHOPS'";
      $sql .= "order by date DESC";
      $result = self::$database->query($sql);
      if (!$result) {
          exit("Database query failed.");
  ...
  static public function find_by_past_event_type($event_type)
  {
      $sql = "SELECT * FROM " . self::$table_name . " ";
      $sql .= "WHERE event_type='" . self::$database->escape_string($event_type) . "'";
  ...
  static public function find_by_single_web_event() {
    $sql = "SELECT * FROM " . static::$table_name . " ";
     $obj_array = static::find_by_sql($sql);
    if(!empty($obj_array)) {
      return array_shift($obj_array);
